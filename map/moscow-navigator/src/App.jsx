...
function App() {
  ...
  const handleRouteSearch = async (transportMode = 'walking', totalTime = null, waypoints = []) => {
    ...
    try {
      ...
      const createExtendedRoute = async (from, to, targetTimeMinutes) => {
        const targetDistanceKm = (targetTimeMinutes / 60) * 3 // 3 км/ч средняя скорость ходьбы
      ...
      // Функция для построения маршрута сегментами
      const buildRouteInSegments = async (points, mode) => {
        console.log('🔗 Строим маршрут сегментами через', points.length, 'точек')
        ...
        for (let i = 0; i < points.length - 1; i++) {
          ...
          console.log(`📍 Сегмент ${i + 1}: от точки ${i + 1} к точке ${i + 2}`)

          try {
            const segmentRoute = await buildRoute(from, to, mode)

            // Пересчитываем время для пешеходной скорости
            const recalculatedSegmentDuration = recalculateWalkingTime(segmentRoute.distance)

            segments.push({
              from: from,
              to: to,
              distance: segmentRoute.distance,
              duration: recalculatedSegmentDuration, // используем пересчитанное время
              coordinates: segmentRoute.geometry.coordinates
            })

            totalDistance += segmentRoute.distance
            totalDuration += recalculatedSegmentDuration // используем пересчитанное время

            // Добавляем координаты сегмента (исключаем первую точку для избежания дублирования)
            ...
            console.log(`✅ Сегмент ${i + 1}: ${(segmentRoute.distance / 1000).toFixed(1)} км, ${Math.round(recalculatedSegmentDuration / 60)} мин (пересчитано для 3 км/ч)`)

          } catch (error) {
            console.error(`❌ Ошибка построения сегмента ${i + 1}:`, error)
            throw new Error(`Не удалось построить сегмент маршрута ${i + 1}`)
          }
        }

        console.log(`🎯 Общий маршрут: ${(totalDistance / 1000).toFixed(1)} км, ${Math.round(totalDuration / 60)} мин`)
      ...
      // Функция для геокодирования промежуточных точек
      const geocodeWaypoints = async (waypoints) => {
        const geocodedWaypoints = []

        for (const waypoint of waypoints) {
          try {
            console.log('🔍 Геокодируем промежуточную точку:', waypoint.address)
            const response = await fetch(
              `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(waypoint.address + ', Москва, Россия')}&limit=1&addressdetails=1`
            )

            if (!response.ok) {
              console.warn('⚠️ Ошибка при геокодировании точки:', waypoint.address)
              continue
            }
            ...
            if (data && data.length > 0) {
              ...
              if (!isNaN(coords[0]) && !isNaN(coords[1])) {
                ...
                console.log('📍 Промежуточная точка найдена:', waypoint.address, coords)
              }
            }
          } catch (error) {
            console.warn('⚠️ Ошибка геокодирования точки:', waypoint.address, error)
          }
        }
      ...
      if (waypoints && waypoints.length > 0) {
        console.log(`🛑 Строим маршрут через ${waypoints.length} промежуточных точек`)

        // Геокодируем промежуточные точки
        geocodedWaypoints = await geocodeWaypoints(waypoints)
        console.log(`📍 Успешно геокодировано ${geocodedWaypoints.length} из ${waypoints.length} точек`)

        if (geocodedWaypoints.length > 0) {
          // Строим маршрут сегментами: начало -> промежуточные -> конец
          const allPoints = [fromCoords, ...geocodedWaypoints.map(wp => wp.coordinates), toCoords]
          console.log('🗺️ Строим маршрут сегментами через точки:', allPoints.length)
          routeInfo = await buildRouteInSegments(allPoints, transportMode)
        } else {
          console.log('⚠️ Не удалось геокодировать промежуточные точки, строим прямой маршрут')
          routeInfo = await buildRoute(fromCoords, toCoords, transportMode)
        }
      } else if (totalTime && totalTime > 0) {
      ...
      // Исправляем координаты - OSRM возвращает [lon, lat], а нам нужно [lat, lon]
      const correctedCoordinates = routeInfo.geometry.coordinates.map(coord => [coord[1], coord[0]])

      console.log('🗺️ Первые 3 точки маршрута:', correctedCoordinates.slice(0, 3))
      console.log('🗺️ Последние 3 точки маршрута:', correctedCoordinates.slice(-3))
      console.log('📏 Расстояние от API:', routeInfo.distance, 'метров')
      console.log('⏱️ Время от API:', routeInfo.duration, 'секунд')

      // Пересчитываем время для пешеходной скорости 3 км/ч
      const recalculatedDuration = recalculateWalkingTime(routeInfo.distance)

      // Если есть сегменты, пересчитываем время для каждого
      const recalculatedSegments = routeInfo.segments ? routeInfo.segments.map(segment => ({
        ...segment,
        duration: recalculateWalkingTime(segment.distance)
      })) : []

      setRoute({
        coordinates: correctedCoordinates,
        distance: routeInfo.distance,
        duration: recalculatedDuration, // используем пересчитанное время
        from: fromCoords,
        to: toCoords,
        waypoints: geocodedWaypoints, // используем геокодированные точки
        segments: recalculatedSegments // сегменты с пересчитанным временем
      })

      // Центрируем карту на начальной точке маршрута для лучшего отображения
      console.log('🎯 Центрируем карту на начальной точке:', fromCoords)
  ...
  // Функция для пересчета времени OSRM для пешеходов (3 км/ч)
  const recalculateWalkingTime = (distanceMeters) => {
    // OSRM возвращает время для автомобилей, пересчитываем для пешеходов
    const distanceKm = distanceMeters / 1000
    const walkingSpeedKmh = 3 // км/ч
    const timeHours = distanceKm / walkingSpeedKmh
    const timeSeconds = timeHours * 3600

    console.log(`⏱️ Пересчет времени: ${distanceKm.toFixed(2)} км при скорости ${walkingSpeedKmh} км/ч = ${Math.round(timeSeconds / 60)} мин`)
  ...
  return (
    <div className="app">
      ...
      <div className={`map-container ${isAuthModalOpen ? 'map-hidden' : ''}`}>
        {!isAuthModalOpen && (
          <>
            {console.log('🗺️ App - Передаем в MapComponent:', {
              center: mapCenter,
