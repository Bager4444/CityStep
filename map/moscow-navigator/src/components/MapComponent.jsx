...
// Добавляем стили для маркеров промежуточных точек
...
if (typeof document !== 'undefined') {
  ...
  styleElement.textContent = waypointStyles
...
function RemainingInfoPanel({ route, arrowPosition }) {
  ...
  useEffect(() => {
    ...
    // Рассчитываем оставшееся время (средняя скорость пешехода 3 км/ч = 0.833 м/с)
    const walkingSpeedMps = 0.833 // метров в секунду
...
function MapComponent({
  ...
  // Автоматическое подгонка карты под маршрут
  useEffect(() => {
    if (!mapRef.current || !route?.coordinates || route.coordinates.length < 2) return

    try {
      const map = mapRef.current
      if (map && map.fitBounds) {
        // Создаем границы для всех точек маршрута
        const bounds = route.coordinates.reduce((bounds, coord) => {
          return bounds.extend(coord)
        }, L.latLngBounds(route.coordinates[0], route.coordinates[0]))

        // Подгоняем карту под границы маршрута
        map.fitBounds(bounds, { padding: [20, 20] })
        console.log('🗺️ Карта подогнана под маршрут')
      }
    } catch (error) {
  ...
  console.log('🗺️ MapComponent - validCenter:', validCenter)
  console.log('🗺️ MapComponent - route:', route ? `${route.coordinates?.length} точек` : 'нет маршрута')
  console.log('🗺️ MapComponent - markers:', markers?.length || 0)

  // Проверяем валидность координат маршрута
  if (route?.coordinates) {
    const invalidCoords = route.coordinates.filter(coord =>
    ...
    if (invalidCoords.length > 0) {
      console.error('🚫 Найдены невалидные координаты:', invalidCoords.slice(0, 3))
    }
  }
    ...
    <div className={`map-wrapper ${className || ''}`}>
      ...
      >
        ...
        {route && route.coordinates && route.coordinates.length > 0 ? (
          <>
            {console.log('🗺️ Рендерим маршрут с координатами:', route.coordinates.length, 'точек')}
            {console.log('🗺️ Первая координата:', route.coordinates[0])}
            {console.log('🗺️ Последняя координата:', route.coordinates[route.coordinates.length - 1])}
            ...
            {route.waypoints && route.waypoints.map((waypoint, index) => (
              <Marker
                ...
                icon={L.divIcon({
                  ...
                  html: `<div class="waypoint-marker-content">${index + 1}</div>`,
                  iconSize: [24, 24],
                  iconAnchor: [12, 12]
                })}
              >
                <Popup>
                  <strong>Остановка {index + 1}</strong>
                  <br />
                  {waypoint.address}
                </Popup>
              </Marker>
          ...
          console.log('🚫 Маршрут не отображается:', {
            hasRoute: !!route,
